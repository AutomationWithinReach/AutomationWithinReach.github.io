...
<!-- *************************************************************** -->
	...
	How to change models:
		...
		Lines 122 to 140 all follow the same Format:  <option value="models/AWR-00096.glb">AWR-00096_Rails, Pairs with AWR-00097</option>
		To add or remove a model from the site simply add or delete a line with this format  <option value="path to model">Name of Model</option>

		Note: If your are changing the first model in the list then the path on line 66 will also need to be changed. <model-viewer id="dimension-demo" poster="awr-logo.png" loading="auto" reveal="auto" src="CHANGE TO NEW PATH" ar ar-modes="webxr scene-viewer quick-look" camera-controls tone-mapping="commerce" shadow-intensity="1">
	...
	How to change hotspots:
		...
		Next find the <model-viewer> snippet which is the code at the top of the ribbon.
		After adding hotspots the snippet will include button classes similar to this: 
			<button class="Hotspot" slot="hotspot-1" data-position="0.3934389680899433m 1.190869892043453m 0.08253260496446702m" data-normal="-0.31274927296837296m -0.09642783343201293m 0.9449283386571502m" data-visibility-attribute="visible">
		...
		Next add the button classes from the snippet underneath the last hotspot on line 114. 
		Note you need to rename and number the hotspot ID, class, and slot along with the annotation ID to follow the nameing convention
		Next find the clearAnnotations() function on line 259 remember after adding the previous code this line number is no longer accuate.
		...
		Next find the line containing: var modelPath = document.getElementById("src").value; which is currently line 294
